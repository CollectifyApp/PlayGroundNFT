    ...
    context(`${contract}`, function () {
      ...
      describe('supports interface specs', async function () {
        ...
        it('supports AccessControl', async function () {
          expect(await this.erc721token.supportsInterface('0x7965db0b')).to.eq(true);
        });

        it('does not support ERC721Enumerable', async function () {
          expect(await this.erc721token.supportsInterface('0x780e9d63')).to.eq(false);
        });
      ...
      context('set mint time', async function () {
        ...
        it('Only the owner can modify it', async function () {
          const timestamp = await getBlockTimestamp()
          await expect(this.erc721token.connect(this.addr1).changePrivateMintTime([timestamp, timestamp + 100])).to.be.revertedWith('error: 20000 - only owner can call this function');
          await expect(this.erc721token.connect(this.addr1).changePublicMintTime([timestamp + 100, timestamp + 200])).to.be.revertedWith('error: 20000 - only owner can call this function');
        })
      })
